...
from models.CD_Ister.layers.Transformer_EncDec import Encoder, EncoderLayer
from models.CD_Ister.layers.SelfAttention_Family import AttentionLayer
from models.CD_Ister.layers.Embed import DataEmbedding_inverted
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.Linear_Seasonal = nn.Linear(configs.d_model, self.label_len + self.pred_len, bias=True)
            self.Linear_Trend = nn.Linear(configs.d_model, self.label_len + self.pred_len, bias=True)

            self.Linear_Seasonal.weight = nn.Parameter(
                (1 / configs.d_model) * torch.ones([self.label_len + self.pred_len, configs.d_model]))
