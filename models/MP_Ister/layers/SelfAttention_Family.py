...
class DotAttentionLayer(nn.Module):
    def __init__(self, d_model, attention_dropout=0.1, d_keys=None,
                 d_values=None):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        scores = torch.sum(self.gelu(queries * keys), dim=1, keepdim=True).repeat(1, L, 1, 1)

        # Output
        V = self.dropout(values * scores)

        return self.out_projection(V), queries.sum(dim=-1).sum(dim=-1)[0, :]
